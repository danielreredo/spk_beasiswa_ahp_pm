...
@section('title')
<h1 class="m-0 text-dark">Ubah Data Kepala Program Studi</h1>
@endsection
...
<div class="container">
    <div class="row">
        <div class="col-7">
        <form method="post" action="{{ url('/opes') }}/{{$ope->id_ope}}">
        @method('patch')       
        @csrf
        <div class="form-group">
            <label for="nama">Nama Kepala Program Studi</label>
            <input type="text" class="form-control @error ('nm_ope') is-invalid @enderror" id="nm_ope" placeholder="Masukkan Nama" name="nm_ope" value="{{$ope->nm_ope}}">
            @error('nm_ope')
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control @error ('nidn') is-invalid @enderror" id="nidn" placeholder="Masukkan Nama" name="nidn" value="{{$ope->nidn}}">
            @error('nidn')
            <div class="invalid-feedback">{{$message}}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            @if($ope->jk_ope === 'Laki-laki')
            <option value="Laki-laki" selected>Laki-laki</option>
            <option value="Perempuan">Perempuan</option>
            @endif
            @if($ope->jk_ope === 'Perempuan')
        ...
        <div class="form-group">
            ...
            <option value="{{$prodi->id_pro}}"{{$ope->pro===$prodi->id_pro?' selected':''}}>{{$prodi->nm_pro}}</option>
            @endforeach
            </select>
