...
<div class="container">
    <div class="row">
        <div class="col-7">
        <form method="post" action="{{ url('/pers') }}/{{$per->id_per}}">
        @method('patch') 
        @csrf
        <div class="form-group">
            <label for="nama">Periode</label>
            <input type="text" class="form-control @error ('per') is-invalid @enderror" id="per" placeholder="Masukkan Periode" name="per" value="{{$per->per}}">
            @error('per')
